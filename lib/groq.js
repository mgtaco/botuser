...
- If you include an image link from the reference info, make the item name itself the Markdown link, like [Item Name](image-url).
- Keep image links as part of the relevant item text, not as separate URL parts.
- Never use Markdown image syntax like ![name](url), because Discord will not render it correctly.
- Discord markdown is allowed when useful: **bold**, *italic*, \`inline code\`, \`\`\`code blocks\`\`\`, > quotes, lists, [links](url), and ||spoilers||.
- Do not use Markdown tables because Discord does not render them.`,
  ];
